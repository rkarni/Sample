...
aws deploy push --application-name %AWSCodeDeployApplicationName% --description "%DLLVersionNumber%_%localrevisionnumber%_%Environment%" --s3-location s3://%S3Bucket_Prod%/%Application_Name%/%Environment%/%DLLVersionNumber%_%localrevisionnumber%/%Folder_Name%/%Folder_Name%.zip --source %workspace%\Artifacts\%Environment%\%Application_Name%\%Folder_Name%
